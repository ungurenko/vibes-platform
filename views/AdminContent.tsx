...
import { COURSE_MODULES, STYLES_DATA, PROMPTS_DATA, GLOSSARY_DATA, ROADMAPS_DATA, DASHBOARD_STAGES, PROMPT_CATEGORIES_DATA } from '../data';
import { Lesson, StyleCard, PromptItem, PromptCategoryItem, GlossaryTerm, CourseModule, Roadmap, RoadmapStep, DashboardStage } from '../types';
             ...
             <>
                <div>
                   ...
                   <div className="space-y-4">
                      ...
                      <FileUploader
                          ...
                          path="styles"
                          label={editingItem?.image ? "Заменить обложку (JPG/PNG)" : "Загрузить обложку (JPG/PNG)"}
                      />
                ...
                <div>
                   ...
                   <Input
                     value={editingItem?.description || ''}
                     onChange={(e) => updateField('description', e.target.value)}
                     placeholder="Например: Тихая роскошь и сдержанность"
                   />
                </div>

                {/* Long Description */}
                <div>
                   <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-2">Развёрнутое описание</label>
                   <textarea
                     value={editingItem?.longDescription || ''}
                     onChange={(e) => updateField('longDescription', e.target.value)}
                     placeholder="Подробное описание стиля для модального окна..."
                     rows={4}
                     className="w-full px-4 py-3 rounded-xl bg-zinc-50 dark:bg-zinc-800 border border-zinc-200 dark:border-white/10 focus:outline-none focus:border-violet-500"
                   />
                </div>
                ...
                <div>
                   ...
                   <Input
                     ...
                     placeholder="Premium, Serif, Warm"
                   />
                </div>

                {/* Gradient */}
                <div>
                   <label className="block text-sm font-bold text-zinc-700 dark:text-zinc-300 mb-2">CSS градиент (Tailwind классы)</label>
                   <Input
                     value={editingItem?.gradient || ''}
                     onChange={(e) => updateField('gradient', e.target.value)}
                     placeholder="from-amber-100 to-stone-200"
                   />
                </div>
             </>
         )}
